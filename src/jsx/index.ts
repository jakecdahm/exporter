...
// Fallback for Premiere Pro if app name detection fails.
try {
  if (!host[ns] && typeof app !== "undefined") {
    //@ts-ignore
    const hasProject = typeof app.project !== "undefined";
    //@ts-ignore
    const hasEncoder = typeof app.encoder !== "undefined";
    if (hasProject && hasEncoder) {
      host[ns] = ppro;
    }
  }
} catch (error) {}
