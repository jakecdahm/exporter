  ...
  return new Promise(function (resolve, reject) {
    ...
    csi.evalScript(
      `try{
          ...
          if (!host || !host["${ns}"]) {
            JSON.stringify({ name: "Error", message: "Extension namespace not available." });
          } else if (!host["${ns}"]["${functionName}"]) {
            JSON.stringify({ name: "Error", message: "ExtendScript function not found: ${functionName}" });
          } else {
...
export const initBolt = (log = true) => {
  const hasCep = !!(window.cep || window.__adobe_cep__);
  if (hasCep) {
    ...
    if (fs.existsSync(jsxSrc)) {
      ...
      console.warn("No ExtendScript bundle found in extension folder.");
    }
    initializeCEP();
  } else if (log) {
    console.warn("CEP environment not detected; skipping ExtendScript init.");
  }
};
